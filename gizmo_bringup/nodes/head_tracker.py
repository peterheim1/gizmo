...
"""
    head_tracker.py - Version 1.0 2017-10-29
    
    Move the head to track an object pose published on the /target_pose PoseStamped topic.
...
FAKE ='false'
TRACKER_TYPE = rospy.get_param('~tracker_type', 'position')
CONTROLLER_TYPE = 'dyna'
...
class HeadTracker():
    def __init__(self):
        ...
        if TRACKER_TYPE == 'position':
            rospy.Subscriber('target_pose', PoseStamped, self.update_joint_positions, queue_size=1)
        else:
            rospy.Subscriber('target_pose', PoseStamped, self.update_joint_speeds, queue_size=1)
    ...
    def center_head_servos(self):
        ...
        self.set_servo_position(self.head_tilt_joint, 0)
        self.set_servo_position(self.head_pan_joint, 0)
    ...
    def shutdown(self):
        ...
        rospy.loginfo("to be detailed Relaxing pan and tilt servos.")
