...
class JointStatePublisher():
    def __init__(self):
        rospy.init_node('robbie_joint_state_publisher', anonymous=True)
        ...
        #rospy.logwarn(self.joints) 
        for controller in sorted(self.joints):
            self.joint_states[controller] = JointStateMessage(controller, 0.0)
            self.controllers.append(controller)
                           
        # Start controller state subscribers
        [rospy.Subscriber(c + '/state', JointStateDY, self.controller_state_handler) for c in self.controllers]
    ...
    def controller_state_handler(self, msg):
        js = JointStateMessage(msg.name, msg.current_pos,  )
