...
def create_root():
    ...
    dock2bb = py_trees_ros.subscribers.EventToBlackboard(
        name="Dock2BB",
        topic_name="/dashboard/auto_dock",
        variable_name="event_auto_dock_button"
    )
    ...
    auto_dock = py_trees.composites.Sequence(name="Auto_dock")

    #########################  nav_home
    nav_home = py_trees.composites.Sequence(name="Nav_home")
    is_nav_requested = py_trees.blackboard.CheckBlackboardVariable(
        name="Nav_dock?",
        variable_name='event_auto_dock_button',
        expected_value=True
    )
    home = py_trees_ros.actions.ActionClient(
        ...
        action_goal= create_nav_goal(0.519, 0.126, 2.0),
        override_feedback_message_on_running="going home"
     )
    ######################### scan
    foyer = py_trees.composites.Sequence(name="Foyer")
    ...
    nav_play = py_trees_ros.actions.ActionClient(
        name="Play_ground",
        action_namespace="/move_base",
        action_spec=move_base_msgs.MoveBaseAction,
        action_goal= create_nav_goal(2.818, 3.090,-2.0),
        override_feedback_message_on_running="going to play ground"
    )

    charging = py_trees.timers.Timer("Pause", duration=5.0)
    idle = py_trees.behaviours.Running(name="Idle")

    ################### tree
    root.add_children([topics2bb, priorities])
    topics2bb.add_children([dock2bb, scan2bb, battery2bb])
    priorities.add_children([battery_check, foyer, nav_home, idle])
    battery_check.add_children([is_battery_ok, auto_dock])
    auto_dock.add_children([home, charging])
    nav_home.add_children([is_nav_requested,home])
    foyer.add_children([is_scan_requested, nav_play])
    return root
